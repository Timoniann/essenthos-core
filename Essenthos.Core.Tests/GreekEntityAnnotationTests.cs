...
public sealed class GreekEntityAnnotationTests : IDisposable
{
    ...
    /// <summary>
    /// One record bears G3575 and nothing was ruled out to reach Noah, which is what
    /// <c>strong-number</c> asserts and what it is for.
    /// </summary>
    [Fact]
    public async Task AGreekNameNothingHadToChooseBetweenKeepsTheNumberAsItsMethod()
    {
        await _loader.Load();

        var noah = await _db.WordEntities.SingleAsync(a => a.WordId == Greek(1).Id);
        noah.Method.Should().Be(LinkMethod.StrongNumber);
    }

    /// <summary>
    /// And the Greek half of what the peoples taught the Hebrew half. Three men bear G2197 and the
    /// word is the third only because the encyclopedia names the other two nowhere the Greek
    /// reaches — so something was ruled out, and <c>strong-number</c>, which says nothing was,
    /// would be a claim of the wrong kind however right the answer. There are 46 such numbers in
    /// the corpus and 122,000 Greek words standing on a name several records bear, so a loader that
    /// asked only the Hebrew column would put back on this side exactly the fault the other side
    /// had just had taken out of it.
    /// </summary>
    [Fact]
    public async Task AGreekNameReachabilityHadToChooseIsWrittenAsTheFormOfTheWord()
    {
        await _loader.Load();

        var zechariah = await _db.WordEntities.SingleAsync(a => a.WordId == Greek(7).Id);
        zechariah.Method.Should().Be(LinkMethod.Lexical);
        zechariah.Source.Should().Contain("named in no book the Greek holds");
    }
    ...
    /// </summary>
    [Fact]
    public async Task TheClaimOnSuchAGreekAnnotationSaysTheSame()
    {
        await _loader.Load();

        var zechariah = await _db.WordEntities.SingleAsync(a => a.WordId == Greek(7).Id);
        var claims = await _db.WordEntityClaims
            .Where(c => c.WordEntityId == zechariah.Id)
            .ToListAsync();
